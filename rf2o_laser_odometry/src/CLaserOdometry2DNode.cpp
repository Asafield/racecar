...
public:
  ...
  bool publish_tf, new_scan_available, debug;
  ...
  CLaserOdometry2D()
{
  ...
  oss << "laser_scan_topic: " << laser_scan_topic << ";\n";
  oss << "odom_topic: " << odom_topic << ";\n";
  oss << "base_frame_id: " << base_frame_id << ";\n";
  oss << "odom_frame_id: " << odom_frame_id << ";\n";
  oss << "publish_tf: " << publish_tf << ";\n";
  oss << "init_pose_from_topic: " << init_pose_from_topic << ";\n";
  oss << "freq: " << freq << ";\n";
  oss << "verbose: " << verbose << ";\n";
  oss << "debug: " << debug << ";\n";
  ...
  else
  {
		...
		pn.param<double>("init_pose_position_x", initial_robot_pose.pose.pose.position.x, 0.0);
		pn.param<double>("init_pose_position_y", initial_robot_pose.pose.pose.position.y, 0.0);
		pn.param<double>("init_pose_position_z", initial_robot_pose.pose.pose.position.z, 0.0);
		pn.param<double>("init_pose_orientation_x", initial_robot_pose.pose.pose.orientation.x, 0.0);
		pn.param<double>("init_pose_orientation_y", initial_robot_pose.pose.pose.orientation.y, 0.0);
		pn.param<double>("init_pose_orientation_z", initial_robot_pose.pose.pose.orientation.z, 0.0);
		pn.param<double>("init_pose_orientation_w", initial_robot_pose.pose.pose.orientation.w, 1.0);
...
bool CLaserOdometry2DNode::setLaserPoseFromTf()
{
  ...
  try
  { 
    // "ERROR: Wait for link transform timeout !!!"
    if(debug){
        std::ostringstream buff("last_scan.header.frame_id = ");
	    buff << last_scan.header.frame_id;
	    ROS_INFO(buff.str().c_str());
    }
    tf_listener.waitForTransform(base_frame_id, last_scan.header.frame_id, ros::Time(0), ros::Duration(0.01), ros::Duration(100.0));
...
void CLaserOdometry2DNode::LaserCallBack(const sensor_msgs::LaserScan::ConstPtr& new_scan)
{
  if (GT_pose_initialized)
  {
	...
	// os << last_scan.header.frame_id << "\nstamp" << last_scan.header.stamp;
...
void CLaserOdometry2DNode::publish()
{
  ...
  if (publish_tf)
  {
    ROS_INFO("[rf2o] Publishing TF: [base_link] to [odom: %s]", odom_frame_id.c_str());
